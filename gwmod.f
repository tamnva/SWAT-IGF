...
!! infiltrate at the same speed as what goes through the soil profile.
      ...
      slow2fast_storage = slow2fast_storage + 
     &   slow2fast * (sepbtm(j)+gwq_ru(j)+rchrg_karst)*hru_ha(ihru)*10
     
      rchrg(j) = (1.-gw_delaye(j))*(1-slow2fast)*(sepbtm(j) + gwq_ru(j)+
     ...
     &                             rchrg_karst) + gw_delaye(j) * rchrg1
      ...
      if (iyr > 1999) then
        write(2467, *) j, (sepbtm(j)+gwq_ru(j)+rchrg_karst)
      end if
...
!! changed by Nguyen (spring_slow_sto in m3)

      if ((snam(ihru)(4:4) == "N") .or. (j > 1086))then
        spring_slow_sto = spring_slow_sto + 
     &                    hru_ha(ihru)* gwseep * s_ifrac * 10 
      end if
