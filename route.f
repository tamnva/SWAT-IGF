...
!! changed by Nguyen
      ...
      spring_fast_sto = spring_fast_sto + qtloss
      
      if (jrch == s_num) then
      
      !convert part of infiltrated water from HRU to fast recharge
      spring_fast_sto = spring_fast_sto + slow2fast_storage
      
      fflow = fflow * exp(-fday) +(1-exp(-fday))* spring_fast_sto/86400
      sflow = sflow * exp(-sday) +(1-exp(-sday))* spring_slow_sto/86400
      
      rtwtr = rtwtr + (fflow + sflow)*86400
      
      tstor = tstor + spring_fast_sto + spring_slow_sto
     &     - (fflow + sflow)*86400
     
      write(2469,'(3(f20.2))') spring_fast_sto, spring_slow_sto, tstor
      ! delay of q is two day, infiltrated water will appear at spring at 3rd day
!!      rtwtr = rtwtr + q_delay(1)
!!      q_delay(1) = q_delay(2)
!!     q_delay(2) = q_delay(3)
!!      q_delay(3) = (fflow + sflow)*86400
       
      !! rtwtr = rtwtr + (fflow + sflow)*86400
      
      end if
      
      !changed by Nguyen (get flow from subbasin Nr.26)
      if (jrch == 26) then
        q26 = rtwtr
      end if
